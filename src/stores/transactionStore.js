...
export const useTransactStore = defineStore('transactionData', {
    ...
    actions: {
        ...
        // New
        async fetchBaristaProductDetailsStore(transactionId) {
            this.loading = true;
            this.error = null;
            try {
                if (!transactionId) {
                ...
                const response = await TRANSACTION_API.fetchBaristaProductDetailsApi(transactionId);
                if (response && response.status === true) {
                    this.orderDtls = response;
                    this.orderDtlsData = response.data;
                    return response;
                } else {
                    throw new Error(response?.message || 'Failed to fetch barista product details');
                }
            } catch (error) {
                console.error('Error fetching barista product details:', error);
                this.error = error.message || 'Failed to fetch barista product details';
                throw error;
            }
            finally {
                this.loading = false;
        ...
        // New
        async updateBaristaProductStatusStore(productId, transactionId, orderStatus) {
            this.loading = true;
            this.error = null;
            try {
                ...
                const response = await TRANSACTION_API.updateBaristaProductStatusApi(productId, transactionId, orderStatus);
                if (response && response.status === true) {
