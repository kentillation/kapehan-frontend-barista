...
export const TRANSACTION_API = {
    ENDPOINTS: {
        FETCH_CURRENT_ORDERS: '/barista/current-orders',
        FETCH_STATION_STATUS: '/barista/station-status',
        FETCH_BARISTA_PRODUCT_DETAILS: '/barista/barista-product-details',
        UPDATE_BARISTA_PRODUCT_ORDER_STATUS: '/barista/update-barista-product-status',
    },
    ...
    async fetchBaristaProductDetailsApi(transactionId) {
        try {
            const authToken = localStorage.getItem('auth_token');
            if (!authToken) {
                throw new Error('No authentication token found');
            ...
            const response = await apiClient.get(
                `${this.ENDPOINTS.FETCH_BARISTA_PRODUCT_DETAILS}/${transactionId}`,
                config
            );
            if (!response.data) {
                throw new Error('Invalid response from server');
            ...
            console.error('[TRANSACTION_API] Error fetching barista product status:', error);
            const enhancedError = new Error(
                error.response?.data?.message ||
                error.message ||
                'Failed to fetch barista product status'
            );
            enhancedError.response = error.response;
            enhancedError.status = error.response?.status;
            throw enhancedError;
