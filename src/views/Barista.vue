    ...
    <v-container>
        ...
        <v-sheet v-if="this.orders.length === 0" class="d-flex flex-column align-center text-center mx-auto mt-5"
            ...
            <div class="mt-14">
                ...
                <p class="text-grey mb-3 mx-3">Looks like there is no order found for barista station.</p>
            </div>
        </v-sheet>
        <v-row class="mt-1">
        ...
        async fetchCurrentOrders() {
            ...
            try {
                ...
                await Promise.all(this.transactStore.currentOrders.map(async (order) => {
                    try {
                        const response = await this.transactStore.fetchBaristaProductDetailsStore(order.transaction_id);
                        if (response?.data) {
                            ...
                            // Check if this order has kitchen items (station_id = 1)
                            if (orderItems.some(item => Number(item.station_id) === 1)) {
